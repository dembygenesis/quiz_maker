...
import (
	"errors"
	"fmt"
	"github.com/dembygenesis/quiz_maker/src/app/common/config"
	"github.com/dembygenesis/quiz_maker/src/app/common/utilities/controller_utils"
	"github.com/dembygenesis/quiz_maker/src/app/common/utilities/date"
	"github.com/dembygenesis/quiz_maker/src/app/common/utilities/error_utils"
	"github.com/dembygenesis/quiz_maker/src/app/common/utilities/gorm_utils"
	"github.com/dembygenesis/quiz_maker/src/app/common/utilities/interface_utils"
	"github.com/dembygenesis/quiz_maker/src/app/common/utilities/response_builder"
	"github.com/dembygenesis/quiz_maker/src/app/common/utilities/string_utils"
	"github.com/dembygenesis/quiz_maker/src/v1/api/models/email"
	"github.com/dembygenesis/quiz_maker/src/v3/api/models"
...
type Repository interface {
	Login(p *RequestLogin) (*ResponseUserInfo, *error_utils.ApplicationError)
...
// Login - does login lol
func (r *RepositoryImpl) Login(p *RequestLogin) (*ResponseUserInfo, *error_utils.ApplicationError) {
	var user modelsV2.User
	...
	err = r.DB.Transaction(func(tx *gorm.DB) error {
		...
		matched := string_utils.Decrypt(user.Password, p.Password)
		if matched == false {
			return errors.New("username/password mismatch")
		}

		var userType models.UserType
		err = tx.Where("id = ?", user.UserTypeID).First(&userType).Error
		if err != nil {
			return err
		}

		token, err := string_utils.MakeJWT(user.ID)
		if err != nil {
		...
		userInfo.Role = userType.Name

		return err
	})
	if err != nil {
		return nil, &error_utils.ApplicationError{
			HttpStatus: http.StatusBadRequest,
			Message:    err.Error(),
			Error:      err,
		}
